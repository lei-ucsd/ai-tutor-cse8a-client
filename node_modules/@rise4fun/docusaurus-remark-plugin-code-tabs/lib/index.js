    ...
    return async (root, vfile) => {
        ...
        (0, unist_util_visit_1.default)(root, 'code', (node, nodeIndex, parent) => {
            if (!parent || visited.has(node))
                return;
            visited.add(node);
            const { tabs, lazy, codesandbox } = parseMeta(node);
            if (tabs === undefined && codesandbox === undefined)
                return;
            const codes = [node];
            ...
            while (tabs !== undefined &&
                ...
                codes.push(parent.children[nodeIndex]);
                nodeIndex++;
            }
            ...
            if (codes.length > 1) {
                ...
                const mdx = [
                    {
                        type: 'jsx',
                        value: `<Tabs ${lazy !== undefined ? 'lazy' : ''} groupId={${JSON.stringify(tabs || 'default')}}>`,
                    },
                ];
                codes.forEach((c) => {
                    const { lang = '' } = c;
                    const { title, file } = parseMeta(c);
                    mdx.push({
                        type: 'jsx',
                        value: `<Tab value={${JSON.stringify(file || title || lang)}} label={${toAttributeValue(title || langTitles?.[lang || ''] || PRISM_LANGUAGES[lang || ''] || undefined)}}>`,
                    });
                    mdx.push({ ...c });
                    mdx.push({
                        type: 'jsx',
            ...
            if (codesandbox !== undefined) {
                ...
                parent.children.splice(nextIndex, 0, {
                    type: 'jsx',
                    value: `<CodeSandboxButton startFile=${JSON.stringify(startFile)} files={${JSON.stringify(files)}} ${codesandbox ? `template={${JSON.stringify(codesandbox)}}` : ''} />`,
                });
                nextIndex++;
        ...
        if (needsTabsImport) {
            injectThemeImport(root, 'TabItem');
            injectThemeImport(root, 'Tabs');
        }
        if (needsCodeSandboxImport) {
            injectThemeImport(root, 'CodeSandboxButton');
        }
    };
};
